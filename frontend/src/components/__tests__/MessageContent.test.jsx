...
describe('MessageContent', () => {
  ...
  it('renders a MonsterPill when a STATBLOCK tag is present', () => {
    render(<MessageContent content={`You encounter a [STATBLOCK]${statblockJson}[/STATBLOCK]`} />)
  ...
  it('opens the StatBlockModal when a MonsterPill is clicked', async () => {
    render(<MessageContent content={`[STATBLOCK]${statblockJson}[/STATBLOCK]`} />)
  ...
  it('handles multiple STATBLOCK tags in one message', () => {
    const second = JSON.stringify({ ...JSON.parse(statblockJson), name: 'Orc' })
    render(
      <MessageContent
