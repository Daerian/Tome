...
export default function AdventureViewer({ code }) {
  ...
  async function loadSection(sectionName) {
    ...
    try {
      ...
      const data = await res.json()
      if (data.error) {
        setSectionContent({ title: sectionName, content: `Error: ${data.error}` })
      } else {
        setSectionContent(data)
      }
    } catch {
      setSectionContent({ title: sectionName, content: 'Failed to load section.' })
    } finally {
      setLoading(false)
    }
  }
  ...
  if (loading && !toc && !sectionContent) {
    return <div style={styles.container}><p style={styles.loading}>Loading adventure...</p></div>
  }
