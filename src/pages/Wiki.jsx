...
export default function Wiki({ campaignId, session, role }) {
  ...
  return (
    <div style={s.wrapper}>
      ...
      {/* Page content */}
      <div style={s.content}>
        {activePage === 'npcs' && <NPCs campaignId={campaignId} session={session} role={role} />}
        {activePage === 'locations' && <Locations campaignId={campaignId} session={session} role={role} />}
        {activePage === 'missions' && <Missions campaignId={campaignId} session={session} role={role} />}
        {activePage === 'treasury' && <Treasury campaignId={campaignId} session={session} role={role} />}
      </div>
    </div>
  )
}
